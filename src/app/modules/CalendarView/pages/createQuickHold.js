...
const CreateQuickBook = ({ EmployeeID, BrokerID }) => {
  ...
  const getPropertyResources = useCallback(async () => {
    const properties = await getPropertyCalendarEventResources(
      EmployeeID,
      BrokerID
    );
  ...
  const onSubmit = (values) => {
    createQuickHold(values, BrokerID, EmployeeID)
      .then((res) => {
  ...
  return (
    <QuickBookAddContainer>
      <div className='addQuickBookContent_heading'>
        <PrimaryHeading>Create Quick Hold</PrimaryHeading>
      </div>
      <div className='addQuickBookContent'>
        {formLoading ? (
          ...
          <Formik
            ...
            validationSchema={
              leadFrom ? quickBookValuesSchema : validationSchema
            }
            onSubmit={onSubmit}
          >
            {(formik) => {
