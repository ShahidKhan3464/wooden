...
const DashbaordTable = ({ UnConfirmedHolds, handlePriceOverride }) => {
    const classes = useStyles()
    console.log(UnConfirmedHolds, 'UnConfirmedHolds')
    ...
    const tableMunuHandler = (value) => {
        console.log(value);
    }

    return (
        <Table aria-label="simple table">
            <TableCell style={{ fontSize: '16px', fontWeight: '700', fontFamily: 'Manrope', color: '#121212' }}>Hold Price</TableCell>
            ...
            <TableBody>
                {UnConfirmedHolds?.map((Hold, index) => (
                    <TableRow
                        key={Hold.name}
                    ...
                    >
                        ...
                        <TableCell style={tableContent}>{`${getFormattedDateFromServer(
                            Hold.checkInDate
                        )} - ${getFormattedDateFromServer(Hold.checkOutDate)}`}</TableCell>
